...
/**
 * =========================================================================
 * BELGİN SAAT — GÜNLÜK SAAT 03:00 CHRONO24 FİYAT & STOK SENKRONİZASYON MOTORU
 * =========================================================================
 * - Çalışma Zamanı: Her gün 03:00 (Cron: 0 3 * * *)
 * - Fiyatlama Formülü: Chrono24 USD Piyasa Fiyatı + %40 (x 1.40) * USD/TRY Kuru
...
const CHRONO24_MARKUP = 1.40; // Zorunlu +%40 kâr marjı
...
function runSync() {
  console.log(`[Chrono24 Sync - 03:00] Başlatılıyor: ${new Date().toISOString()}`);
  console.log(`[Chrono24 Sync] Referans Kur: 1 USD = ${USD_TRY_RATE} TL | Kâr Marjı: +%40 (x${CHRONO24_MARKUP})`);
  ...
  const updatedProducts = PRODUCTS.map(p => {
    if (p.isElite || p.category === 'elit-saatler') {
      ...
      // Re-calculate price with +40% Chrono24 formula
      const baseUsd = Number(p.usdRefPrice) || Math.round(p.price / (CHRONO24_MARKUP * USD_TRY_RATE));
      const targetTry = Math.round(baseUsd * CHRONO24_MARKUP * USD_TRY_RATE);
  ...
  if (valid) {
    console.log(`[Chrono24 Sync] Tüm 10 marka için 20'şer ürün (Toplam 200 Elit Ürün) eksiksiz doğrulandı.`);
  }

  // Update data.js
  const headerComment = `// ==========================================================
// BELGİN SAAT — MASTER ÜRÜN VE KOLEKSİYON VERİTABANI
// Sürüm: ${new Date().toISOString().slice(0, 10)}.chrono24-daily-sync
...
`;
  ...
  console.log(`[Chrono24 Sync] Senkronizasyon başarıyla tamamlandı. (${updatedCount} fiyat güncellendi).`);
}
