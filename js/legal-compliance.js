// BELGIN KUYUMCULUK — legal/compliance controls — 25.08.2026-v2
const LegalCompliance = (() => {
  const HIGH_VALUE_SECURE_DELIVERY_THRESHOLD = 12000;
  const TERMS_VERSION = '2026-08-25-v2';
  const PREINFO_VERSION = '2026-08-25-v2';
  const HIGH_VALUE_VERSION = '2026-08-25-v2';
  const EVIDENCE_VERSION = 'belgin-order-evidence-v2';
  let checkoutPresentedAt = null;
  ...
  function productLegalLinks(){return `<div class="legal-product-links"><a href="iade-degisim.html" target="_blank" rel="noopener">İade ve cayma koşulları</a><a href="garanti-ve-satis-sonrasi.html" target="_blank" rel="noopener">Garanti ve satış sonrası</a><a href="guvenli-odeme-ve-3d-secure.html" target="_blank" rel="noopener">Güvenli ödeme</a><a href="musteri-tanima-ve-islem-guvenligi.html" target="_blank" rel="noopener">Müşteri tanıma ve işlem güvenliği</a><a href="hukuki-delil-ve-kayit-politikasi.html" target="_blank" rel="noopener">Belge ve delil kayıt sistemi</a></div>`;}

  function markPresented(){
    if(!checkoutPresentedAt) checkoutPresentedAt=new Date().toISOString();
    return checkoutPresentedAt;
  }
  ...
  function validateCheckout(){
    ...
    recordConsent('terms_consent',{termsVersion:TERMS_VERSION,preInfoVersion:PREINFO_VERSION,presentedAt:checkoutPresentedAt});
    if(hv)recordConsent('high_value_delivery_consent',{version:HIGH_VALUE_VERSION,internalKycThreshold:HIGH_VALUE_SECURE_DELIVERY_THRESHOLD,presentedAt:checkoutPresentedAt});
  ...
  async function submitCheckout(event){
    ...
    const orderData={
      ...
      legalClientVersions:{terms:TERMS_VERSION,preInformation:PREINFO_VERSION,highValueDelivery:highValue?HIGH_VALUE_VERSION:null,evidence:EVIDENCE_VERSION},
      legalPresentation:{presentedAt:markPresented(),acceptedAt}
    };
    const result=await PayTR.initializePayment(orderData);
    if(!result?.success)return false;
    try{
      localStorage.setItem('last_order_audit',JSON.stringify({orderId:result.merchant_oid,evidenceId:result.evidenceId||null,termsAcceptedAt:acceptedAt,termsVersion:TERMS_VERSION,preInformationVersion:PREINFO_VERSION,highValueDeliveryVersion:highValue?HIGH_VALUE_VERSION:null,totalAmount:(window.Cart?.items||[]).reduce((s,i)=>s+(Number(i.price)||0)*(Number(i.qty)||1),0)}));
    }catch(e){}
    const receipt=document.getElementById('legalOrderReceipt');
    if(receipt)receipt.innerHTML=`<div class="high-value-notice"><strong>Ödeme oturumu oluşturuldu</strong>Sipariş referansı: ${result.merchant_oid}. Delil kayıt kimliği: ${result.evidenceId||'sunucu kaydında'}. ${highValue?'Bu sipariş 12.000 TL ve üzeri iç güvenlik/KYC standardı kapsamında olup yalnız mağazadan kimlik doğrulaması ve imza karşılığı teslim edilecektir.':''}</div>`;
    return false;
  }
  ...
  function saveCookies(mode){
    const prefs={necessary:true,analytics:mode==='all',marketing:mode==='all',updatedAt:new Date().toISOString(),version:'2026-08-25-v2'};
  ...
  return {HIGH_VALUE_SECURE_DELIVERY_THRESHOLD,TERMS_VERSION,PREINFO_VERSION,HIGH_VALUE_VERSION,EVIDENCE_VERSION,isGoldProduct,isWatchProduct,isHighValueProduct,hasHighValueItems,productNotice,productLegalLinks,syncCheckout,validateCheckout,submitCheckout,saveCookies,showCookiePreferences,init};
})();
