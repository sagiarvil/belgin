...
const PayTR = {
  ...
  async initializePayment(orderData) {
    ...
    try {
      ...
      if (!data.success || !data.token || !data.iframeUrl || !data.merchant_oid) {
        throw new Error('Ödeme oturumu doğrulanamadı. İşlem başlatılmadı.');
      }
  ...
  loadIframe(iframeUrl) {
    ...
    if (!container || !wrapper) {
      throw new Error('Ödeme iframe alanı bulunamadı.');
    }

    const parsed = new URL(iframeUrl);
    if (parsed.protocol !== 'https:' || parsed.hostname !== 'www.paytr.com') {
      throw new Error('Geçersiz ödeme sağlayıcı adresi.');
    }
    ...
    iframe.setAttribute('title', 'PayTR Güvenli Ödeme');
    ...
    this._messageHandler = (event) => {
      ...
      try {
        ...
        // PayTR dışı/format dışı mesajlar sessizce yok sayılır.
      }
    };
  ...
  handleResult(orderId, success, message = '') {
    ...
    if (reasonEl) {
      reasonEl.textContent = message || 'Ödeme tamamlanamadı. Bankanız veya ödeme sağlayıcınız işlemi onaylamadı.';
    }
