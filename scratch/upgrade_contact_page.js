...
// 1. Premium Contact Section HTML
const premiumContactHtml = `  <!-- 6. İLETİŞİM & SHOWROOM RANDEVU SAYFASI (ULTRA-PREMIUM SPA) -->
  <section id="page-iletisim" class="page">
    <div class="container-art section-art" style="max-width:1240px; margin:0 auto; padding:40px 20px 80px;">
      ...
      <div style="text-align:center; max-width:820px; margin:0 auto 48px;">
        ...
        <h1 style="font-family:var(--font-serif); font-size:40px; font-weight:700; color:var(--color-ink); margin-bottom:16px; line-height:1.25;">
          İzmir Buca Showroom & VIP Özel Ağırlama Salonu
        </h1>
        <p style="font-size:15.5px; color:#555; line-height:1.8;">
          25 yıldır Menderes Caddesi'ndeki değişmeyen adresimizde, lüks saatler, külçe altın ve sertifikalı mücevher koleksiyonlarımızı özel VIP salonumuzda kahve ikramı eşliğinde güvenle inceleyebilirsiniz.
        </p>
      </div>
...
const oldContactRegex = /<!-- 6\. İLETİŞİM & SHOWROOM RANDEVU SAYFASI \(SPA\) -->[\s\S]*?<\/section>/;
if (oldContactRegex.test(indexHtml)) {
  indexHtml = indexHtml.replace(oldContactRegex, premiumContactHtml);
  fs.writeFileSync('index.html', indexHtml, 'utf8');
  console.log('index.html #page-iletisim updated to ultra-premium design.');
} else {
  console.error('Could not match #page-iletisim in index.html');
}

// 3. Add Premium CSS styles for Contact page in css/style.css
let styleCss = fs.readFileSync('css/style.css', 'utf8');
const premiumContactCss = `
/* ==========================================================
   8. ULTRA-PREMIUM İLETİŞİM SAYFASI STİLLERİ
   ========================================================== */
...
.contact-feature-badge {
  ...
  background: #FAF8F5;
  color: #7A6328;
  border: 1px solid rgba(194,167,104,0.3);
}
...
if (!styleCss.includes('.contact-feature-card')) {
  styleCss += premiumContactCss;
  fs.writeFileSync('css/style.css', styleCss, 'utf8');
  console.log('css/style.css updated with contact-feature-card styles.');
}
