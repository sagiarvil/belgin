...
for file in files {
    ...
    // Scale up by ~2.5x optical factor
    var drawW: CGFloat = 0
    var drawH: CGFloat = 0
    ...
    if let png = rep.representation(using: .png, properties: [:]) {
        ...
        print("Enlarged \(file): drawn \(Int(drawW))x\(Int(drawH)) on \(Int(targetCanvasW))x\(Int(targetCanvasH))")
    }
}
print("All 16 logos enlarged successfully!")
